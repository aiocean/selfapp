<p align="center">
  ...
  <p align="center"><strong>App của bạn. Luật của bạn. Không cần biết code.</strong></p>
  <p align="center">
...
## Vấn đề không ai nói tới

AI viết code được rồi. Ai cũng biết.

Điều ít ai nói là **hàng triệu người** đang dùng AI để "vibe code" app cho mình — theo dõi chi tiêu, ghi chú thói quen, quản lý công việc. Và hầu hết những app đó **chết trong vòng một tuần**.
...
1. **Không biết mô tả cái mình muốn đủ rõ** — và AI xây sai
...
Chúng tôi tin vào một ý tưởng đơn giản:
...
selfapp lấp khoảng trống đó.

---

## Cách hoạt động

selfapp cung cấp cho AI mọi thứ cần thiết để trở thành **kỹ sư phần mềm riêng** của bạn — không phải công cụ bạn ra lệnh, mà là đối tác xây, triển khai và chăm sóc app cho bạn.

### AI là kỹ sư của bạn, không phải công cụ

"Vibe coding" thông thường:

```
Bạn → mô tả app → AI tạo code → bạn tự xoay xở phần còn lại → 💀
```
...
```
Bạn → nói bạn cần gì
  → AI hỏi vài câu thông minh để hiểu ý bạn
  → AI xây app chạy được
  → AI đưa app lên mạng
  → AI ở lại chăm sóc, sửa lỗi, nâng cấp
  → app sống bao lâu tuỳ bạn ✓
```

Khác biệt? **AI không bỏ đi sau khi xây xong.** Nó ở lại làm quản gia cho app của bạn.

### Ba giải pháp cho ba vấn đề

**Vấn đề 1: "Tôi không biết mô tả cái tôi muốn"**
...
selfapp là một **template full-stack mã nguồn mở**, thiết kế để AI đọc, hiểu, và xây dựng tiếp.

```
selfapp/
├── template/           ← Template ứng dụng full-stack
│   ├── fe/             ← Giao diện (Vue 3 + Tailwind CSS)
│   ├── be/             ← Xử lý phía sau (Hono trên Cloudflare Workers)
│   ├── db/             ← Cơ sở dữ liệu (D1 / SQLite)
│   ├── shared/         ← Cấu trúc dữ liệu dùng chung
│   └── .claude/        ← Kỹ năng, agent & quy trình cho AI
│       ├── skills/     ← Hướng dẫn AI cách xây, sửa lỗi, triển khai
│       ├── agents/     ← Chuyên gia AI (kiến trúc sư, reviewer, v.v.)
│       └── commands/   ← Lệnh tắt cho các quy trình thường dùng
└── notes/              ← Nghiên cứu & quyết định thiết kế
```
...
- [Bun](https://bun.sh)
- [Claude Code](https://claude.ai/code) (có Cloudflare MCP)
- Tài khoản Cloudflare (gói miễn phí là đủ)

### Tạo selfapp đầu tiên
...
3. **Kiến thức kỹ thuật không nên là rào cản.** Một giáo viên theo dõi tiến độ học sinh, một đầu bếp quản lý công thức, một freelancer sắp xếp hoá đơn — họ đều xứng đáng có phần mềm riêng mà không cần học code.

4. **UX tốt nhất là cuộc trò chuyện.** Không kéo thả. Không màn hình cấu hình. Chỉ cần nói với AI bạn cần gì, bằng lời của bạn, và nó tự lo phần còn lại.

---

## Trạng thái dự án

selfapp đang được phát triển. Template hoạt động end-to-end (xây → triển khai → vận hành), và chúng tôi đang tinh chỉnh quy trình AI để trải nghiệm mượt mà hơn cho người không biết kỹ thuật.

Mọi đóng góp, phản hồi và ý tưởng đều được hoan nghênh.
...
<p align="center">
  <sub>Được xây dựng với niềm tin rằng ai cũng xứng đáng có phần mềm vừa vặn với cuộc sống mình.</sub>
</p>
