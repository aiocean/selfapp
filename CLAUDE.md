...
6 workflows opinionated:
1. **Khởi tạo** (WF1) — gate check → setup → verify
...
6. **Tiếp tục việc dở** (WF6) — detect state → resume đúng workflow
...
- `workflow/` — Navigator dẫn đường qua từng workflow (khởi tạo, thêm tính năng, sửa lỗi, deploy, giao diện) — chain skills theo thứ tự opinionated
