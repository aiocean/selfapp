...
> **GATE CHECK**: Nếu bất kỳ giá trị nào dưới đây còn là `__PLACEHOLDER__`, AI **PHẢI** chạy quy trình phỏng vấn workflow (skill `setup` Phase 3) để điền thông tin trước khi làm bất kỳ việc gì khác. KHÔNG được code, KHÔNG được setup, KHÔNG được deploy khi chưa hoàn tất block này.

```yaml
app_name: "__PLACEHOLDER__"           # AI tự đặt dựa trên workflow (vd: "Quản lý chi tiêu")
app_goal: "__PLACEHOLDER__"           # Workflow cụ thể app giúp tự động hoá (ngôn ngữ thường)
problem_statement: "__PLACEHOLDER__"  # Pain point chính: phần nào trong quy trình hiện tại khó chịu nhất
current_process: "__PLACEHOLDER__"    # User đang làm việc này bằng cách nào (Excel, giấy, app khác...)
desired_output: "__PLACEHOLDER__"     # Kết quả user muốn THẤY sau khi dùng app
```

### Quy tắc Gate Check

1. **Khi mở conversation mới**, AI đọc block `App Identity` trước tiên
2. Nếu thấy `__PLACEHOLDER__` → chào user và bắt đầu phỏng vấn WORKFLOW ngay, KHÔNG làm gì khác
3. Phỏng vấn hỏi về VIỆC LẶP ĐI LẶP LẠI, không hỏi về features hay tên app
4. Sau khi phỏng vấn xong → AI tự cập nhật block này → build rough draft ngay → chờ user phản hồi
5. Nếu user yêu cầu thay đổi hướng đi của app → cập nhật lại block này trước
...
2. **Xác nhận bằng ngôn ngữ sản phẩm** — "Mình sẽ thêm tính năng tìm kiếm ghi chú, bạn đồng ý không?" thay vì "Mình sẽ thêm RPC method notes.search với full-text query"
3. **Không dùng thuật ngữ kỹ thuật** — Không nói "deploy Worker", nói "đưa app lên mạng". Không nói "migration", nói "cập nhật cấu trúc dữ liệu"
4. **Show-and-refine: xem rồi góp ý, dễ hơn tưởng tượng từ đầu** — Khi user muốn gì mới, build rough draft NGAY (chỉ cần 1-2 tính năng cốt lõi), cho user xem cái THẬT rồi hỏi feedback. KHÔNG bao giờ hỏi user spec chi tiết trước khi bắt tay làm. User react với prototype tốt hơn nhiều so với tưởng tượng từ đầu.
5. **Sửa trước, giải thích sau** — Khi app bị lỗi, sửa TRƯỚC rồi mới giải thích. Không giải thích lúc user đang stress.
...
### Quy trình làm việc — LUÔN đi theo workflow

**Skill `workflow` là trung tâm điều phối của toàn bộ project.** Bất kỳ khi nào user muốn làm gì, AI PHẢI route qua workflow skill để xác định đúng workflow và chain đúng skills. Không bao giờ làm bừa mà không biết mình đang ở workflow nào.

7 workflows opinionated:
1. **Khởi tạo** (WF1) — gate check → interview workflow → setup → rough draft → verify
2. **Thêm tính năng** (WF2) — brainstorm → plan → code → verify → commit
3. **Sửa lỗi** (WF3) — debug → trace → fix → verify → commit
4. **Đưa lên mạng** (WF4) — pre-check → release → verify live
5. **Thay đổi giao diện** (WF5) — brainstorm → design → code → verify → commit
6. **Sửa / cải thiện app** (WF6) — impact check → fix → show → loop until happy → commit
7. **Tiếp tục việc dở** (WF7) — detect state → resume đúng workflow

Mỗi workflow là một chuỗi skill được chain lại — xem chi tiết tại `.claude/skills/workflow/SKILL.md`.

```
User mô tả ý tưởng
  → workflow skill detect intent → chọn workflow phù hợp
  → AI dẫn user qua từng bước trong workflow
  → Mỗi bước invoke skill tương ứng
...
├── shared/
│   └── types.ts                   # Data models (Note, Category, Auth types)
├── be/                            # Workspace: backend
│   ├── package.json               # BE deps (hono)
│   ├── auth.ts                    # Password hashing + session helpers
│   ├── worker.ts                  # Entry point: Hono app + auth middleware + scheduled()
│   └── routes/
│       ├── auth.ts                # Auth routes (setup, login, logout, status)
│       ├── categories.ts          # Category CRUD (multi-entity example)
│       └── notes.ts               # Notes CRUD with category JOIN
├── db/
│   └── migrations/                # SQL migration files (applied via wrangler d1)
│       ├── 001_create_notes.sql
│       ├── 002_create_auth.sql    # Users + sessions tables
│       └── 003_create_categories.sql  # Categories + notes.category_id FK
└── fe/                            # Workspace: frontend (Vue 3 SPA)
    ├── package.json               # FE deps (vue, tailwind, shadcn-vue, vite-plus)
    ├── vite.config.ts
    └── src/
        ├── composables/useApi.ts       # REST API client (auth, notes, categories)
        ├── composables/useAuth.ts      # Auth state management
        ├── composables/useNotes.ts     # Notes state (with category filter)
        ├── composables/useCategories.ts # Categories state management
...
`shared/types.ts` defines data models. BE mounts Hono route groups in `be/worker.ts`, with route handlers in `be/routes/`. FE calls via named methods on the `api` object in `fe/src/composables/useApi.ts`.

**Notes** (with category join): `GET /api/notes?q=&category=`, `GET /api/notes/:id`, `POST /api/notes`, `PUT /api/notes/:id`, `DELETE /api/notes/:id`. FE: `api.notesList(query?, categoryId?)`, `api.notesCreate(input)`, `api.notesUpdate(id, input)`, `api.notesDelete(id)`.

**Categories** (multi-entity relationship example): `GET /api/categories`, `POST /api/categories`, `PUT /api/categories/:id`, `DELETE /api/categories/:id`. FE: `api.categoriesList()`, `api.categoriesCreate(input)`, `api.categoriesUpdate(id, input)`, `api.categoriesDelete(id)`.

**Auth**: `GET /api/auth/status`, `POST /api/auth/setup`, `POST /api/auth/login`, `POST /api/auth/logout`. Public routes (no middleware).
...
3. Mount the route in `be/worker.ts` Hono app (below auth middleware for protected routes)
4. Add a named method to the `api` object in `fe/src/composables/useApi.ts` (e.g. `api.newMethod()`)
5. Add a composable in `fe/src/composables/` for state management (e.g. `useNewEntity.ts`)

## Adding a New Entity with Relationships

Follow the **categories → notes** pattern as a reference for adding related entities:

1. **Migration**: Create table + add FK column to existing table via `ALTER TABLE ... ADD COLUMN`. Use `ON DELETE SET NULL` (keep child, clear FK) or `ON DELETE CASCADE` (delete children). Add index on FK column.
2. **Types**: Define `Entity`, `CreateEntity`, `UpdateEntity` in `shared/types.ts`. If joining, define `EntityWithRelated` using `extends`.
3. **Parent routes** (`be/routes/parent.ts`): Standard CRUD. Delete relies on FK `ON DELETE` behavior.
4. **Child routes**: Use `LEFT JOIN` in SELECT queries to include parent data. Accept optional `parent_id` in create/update. Support `?parent=` filter in list endpoint.
5. **FE composable**: Follow `useCategories.ts` pattern — `ref<T[]>`, fetch/create/update/delete functions.
6. **Mount in worker.ts**: Add `app.route('/api/entities', entityRoutes)` below auth middleware.
...
- `workflow/` — Navigator dẫn đường qua 7 workflow (khởi tạo, thêm tính năng, sửa lỗi, deploy, giao diện, iteration, tiếp tục) — chain skills theo thứ tự opinionated
